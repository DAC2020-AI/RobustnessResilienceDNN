REM run_predtcn_faults.bat > logs/predtcn_faults_log.txt 2>&1

@echo off
set modeltype=predMainTCN
set modelname=predMainTCN_3_13_0.0005
set epsilon=0.3

@echo Train Started: %date% %time%
python experiments/train/train.py -c conf -m %modeltype% -eps 100 -v  -to -sw_name  experiments/train/trained_models/%modelname% -train_with_errors -frate 0.005 -qi 3 -qf 13
@echo Quan: %date% %time%
...
set modelname=predMainTCN_3_13_0.0005_custom_loss
set advname=predMainTCN_3_13_0.0005
set fname=quantized_weights/predMainTCN_3_13_0.0005_custom_loss
@echo Train Started: %date% %time%
python experiments/adversarial-training/adversarial-train.py -c conf -m %modeltype% -eps 100 -v -seed 0 -sw_name adversarial-train-custom-loss/%modelname% -e 0.3 -train_with_errors -frate 0.005 -qi 3 -qf 13
@echo Train Finished: %date% %time%
